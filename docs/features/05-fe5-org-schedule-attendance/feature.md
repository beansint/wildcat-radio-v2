...
- **Timezone — RESOLVED (2026-07-11):** introduced a fixed **station UTC offset** config (`STATION_UTC_OFFSET_MINUTES=480` / `NEXT_PUBLIC_STATION_UTC_OFFSET_MINUTES=480`, Manila UTC+8, no DST). Backend `station-time.ts` (`stationLocalToUtc`/`stationDayWindowUtc`/`stationDateOf`) drives attendance status derivation, the day-window bucketing, and the post-correction date; the frontend mirrors it (`src/lib/time/station.ts`) for the edit dialog + all time display. Cadence weekday-of-date was already tz-independent (left as-is). A pre-08:00 local correction no longer rolls to the previous day (covered by `station-time.spec.ts`).
- **Typed response client — RESOLVED (2026-07-11):** backend endpoints now carry `@ApiOkResponse({ type: XDto })` response DTOs, so orval generates typed responses; the frontend hand-typed shapes (`src/lib/mod/types.ts`, `src/lib/studio/types.ts`) were dropped in favor of generated models (only `Weekday`/`Cadence` kept, since `ShowDto.cadence` is an untyped object in the spec).
- **Show hard-delete — RESOLVED (2026-07-11):** `DELETE /api/shows/:id` now counts episodes first and throws `409 Conflict` if any exist, instead of silently orphaning historical episodes via `ON DELETE SET NULL`.
...
- **Overnight / cross-midnight shows (limitation):** both the show form and the backend `parseCadence` enforce `start < end`, and the weekly daypart grid can't represent a slot spanning midnight, so a `22:00–02:00` show can't be scheduled. Low impact for a campus station (shows run within operating hours). Supporting it needs a cross-midnight cadence rule + grid rendering — **follow-up.**
- **One-time shows deferred from the UI:** the recurrence picker is WEEKLY-only (Mon-Wed-Fri / Daily / Custom) this slice — the weekly grid only renders WEEKLY shows, so exposing `ONE_TIME` would create unmanageable (invisible) shows. The `Cadence` type still supports `ONE_TIME` for a future date-based schedule view. **Follow-up.**
